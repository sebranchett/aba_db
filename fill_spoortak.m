...
folder = fullfile(pwd, 'SpoortakkenCollectie');
fprintf('folder is: %s\n', folder);
files = dir(fullfile(folder, 'SPOORTAK_*.mat'));

for k = 1:numel(files)
    fname = files(k).name;                          % filename only
    fpath = fullfile(folder, fname);                % full path to file
    ...
    [~, version, ~] = fileparts(fname);  % strip of the .mat part
